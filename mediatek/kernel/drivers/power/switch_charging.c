...
#define FULL_CHECK_TIMES		6
 ...
 kal_uint32 g_usb_state = USB_UNCONFIGURED;
  ...
  int g_temp_status=TEMP_POS_23_TO_POS_45;
  kal_bool temp_error_recovery_chr_flag =KAL_TRUE;
  int CCI_recovery_time_high = 0;
  int CCI_recovery_time_cool = 0;
  int CCI_recovery_time_cold = 0;
#endif

extern int CCI_Charger_Switch;
extern int cci_tsbattery_get_temp(void);
int CCI_TEMP_TEST=0;
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
    ...
    else if(g_temp_status == TEMP_POS_23_TO_POS_45)
    {
    	#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
        cv_voltage = BATTERY_VOLT_04_340000_V;
      #else
        cv_voltage = JEITA_TEMP_POS_23_TO_POS_45_CV_VOLTAGE;
      #endif
    }
    else if(g_temp_status == TEMP_POS_10_TO_POS_23)
    {
        cv_voltage = JEITA_TEMP_POS_10_TO_POS_23_CV_VOLTAGE;
    }
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (CCI_recovery_time_high>0)
	{
		CCI_recovery_time_high--;
	}
	if (CCI_recovery_time_cool>0)
	{
		CCI_recovery_time_cool--;
	}
	if (CCI_recovery_time_cold>0)
	{
	...
	else if(BMT_status.temperature > TEMP_POS_23_THRESHOLD)
    {
        if( ((g_temp_status == TEMP_POS_45_TO_POS_60) && (BMT_status.temperature > TEMP_POS_45_THRES_MINUS_X_DEGREE)) ||
            ((g_temp_status == TEMP_POS_23_TO_POS_45 ) && (BMT_status.temperature <= TEMP_POS_23_THRES_MINUS_X_DEGREE ))) 
        {
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Temperature g_temp_status still %d\n\r",g_temp_status);     
        }
        else
        {
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Normal Temperature between %d and %d !!\n\r", TEMP_POS_23_THRESHOLD,TEMP_POS_45_THRESHOLD); 
            g_temp_status = TEMP_POS_23_TO_POS_45;
        }
    }
    else if(BMT_status.temperature > TEMP_POS_10_THRESHOLD)
    {
        if( ((g_temp_status == TEMP_POS_23_TO_POS_45) && (BMT_status.temperature > TEMP_POS_23_THRES_MINUS_X_DEGREE)) ||
        ...
        {
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Normal Temperature between %d and %d !!\n\r", TEMP_POS_10_THRESHOLD,TEMP_POS_23_THRESHOLD); 
            g_temp_status = TEMP_POS_10_TO_POS_23;
        }
    }
...
static void set_jeita_charging_current(void)
{
	...
	if(BMT_status.charger_type == STANDARD_HOST)
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] g_temp_status : %d, BMT_status.bat_vol : %d\r\n", g_temp_status,BMT_status.bat_vol);
	if(g_temp_status ==  TEMP_POS_45_TO_POS_60)
	{
		g_temp_CC_value = CHARGE_CURRENT_800_00_MA;//789mA 0.3C
//		g_temp_input_CC_value = CHARGE_CURRENT_750_00_MA;//910mA
	}
	else if(g_temp_status == TEMP_POS_23_TO_POS_45)
	{
		g_temp_CC_value = CHARGE_CURRENT_1400_00_MA;//1275mA 0.5C
	}
	else if(g_temp_status == TEMP_POS_10_TO_POS_23)
	{
		if(BMT_status.bat_vol >= 4100){
//			g_temp_CC_value = CHARGE_CURRENT_500_00_MA;//667mA 0.2C
			g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;//500mA 0.2C
		}else{
			g_temp_CC_value = CHARGE_CURRENT_1400_00_MA;//1275mA 0.5C
		}
	}
	...
	{
		if(BMT_status.bat_vol >= 4100){
//			g_temp_CC_value = CHARGE_CURRENT_500_00_MA;//667mA 0.1C
			g_temp_input_CC_value = CHARGE_CURRENT_100_00_MA;//100mA 0.1C
		}else{
//			g_temp_CC_value = CHARGE_CURRENT_500_00_MA;//667mA
			g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;//500mA 0.2C
		}
	}else if(g_temp_status == TEMP_NEG_10_TO_POS_0)
	{
	...
	if(BMT_status.charger_type == STANDARD_HOST && g_temp_status == TEMP_POS_0_TO_POS_10 && BMT_status.bat_vol >= 4100){
		g_temp_input_CC_value = CHARGE_CURRENT_100_00_MA;//100mA 0.1C
	}
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] JEITA CC mode charging : %d, input current = %d\r\n", g_temp_CC_value,g_temp_input_CC_value);
...
static kal_uint32 charging_full_check(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] charging full check, get charging status = %d, g_full_check_count = %d\n\r", status, g_full_check_count);
...
static void pchr_turn_on_charging (void)
{
    ...
    else
    {
        ...
        {    
			...
			#if defined(MTK_JEITA_STANDARD_SUPPORT)
			if ( CCI_recovery_time_high > 0 || CCI_recovery_time_cool >0 || CCI_recovery_time_cold >0 || CCI_Charger_Switch == 0) 
			{
				battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] CCI_recovery_time, turn OFF charging !\nCCI_recovery_time_high=%d,CCI_recovery_time_cool=%d,CCI_recovery_time_cold=%d",CCI_recovery_time_high,CCI_recovery_time_cool,CCI_recovery_time_cold);
				charging_enable = KAL_FALSE;
			}
			#endif

       cci_pmic_temp=cci_tsbattery_get_temp();
       battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] PMIC TEMP : %d\r\n", cci_pmic_temp);
       if (cci_pmic_temp<=63000){
               CCI_TEMP_TEST=0;
       }
       if (cci_pmic_temp>=66000 || CCI_TEMP_TEST==1){
