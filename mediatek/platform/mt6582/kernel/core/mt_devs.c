...
unsigned char cci_country_code[4];
unsigned char cci_customer_SN[64] ={0};

/* BUG1035 - HW/PROJECT ID Detection */
int cci_hw_id = 0;
int cci_rf_id = 0;
int cci_hw_id_read(char *page, char **start, off_t off, int count, int *eof, void *data);
int cci_rf_id_read(char *page, char **start, off_t off, int count, int *eof, void *data);
...
int cci_hw_id_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
       ...
       switch(cci_hw_id){
        case EVT:
                len += sprintf(page, "EVT\n");
                break;
        case DVT1:
                len += sprintf(page, "DVT1\n");
                break;
        case DVT2:
                len += sprintf(page, "DVT2\n");
                break;
        case PVT:
                len += sprintf(page, "PVT\n");
                break;
        case MP:
                len += sprintf(page, "MP\n");
                break;
        default:
                len += sprintf(page, "HW_ID_INVALID\n");
                printk(KERN_EMERG "Got an invalid HW version %d\n", cci_hw_id);
                break;
        }
        return len;
}
...
int cci_rf_id_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        ...
        switch(cci_rf_id){
        case VV36:
                len += sprintf(page, "VV36\n");
                break;
        case VV37:
                len += sprintf(page, "VV37\n");
                break;
        case VV38:
                len += sprintf(page, "VV38\n");
                break;
        case VV39:
                len += sprintf(page, "VV39\n");
                break;
        default:
                len += sprintf(page, "RF_ID_INVALID\n");
                printk(KERN_EMERG "Got an invalid RF id %d\n", cci_rf_id);
                break;
        }
        return len;
}
...
{
        sscanf(cci_countrycode, "%s", &cci_country_code);
        printk(KERN_ERR "Read CCI COUNTRY CODE =%s from cmdline!!\n", cci_country_code);

        return 1;
}
__setup("CCI_COUNTRY_CODE=", cci_country_code_from_cmdline);
int cci_country_code_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        int len = 0;
        len += sprintf(page, "%s", cci_country_code);
        printk(KERN_EMERG "Got an invalid Country code = %s\n", cci_country_code);

        return len;
}
static int __init cci_customer_SN_from_cmdline(char *cci_customerSN)
{
        sscanf(cci_customerSN, "%s", &cci_customer_SN);
        printk(KERN_ERR "Read CCI CUSTOMER SN =%s from cmdline!!\n", cci_customer_SN);

        return 1;
}
__setup("CUSTOMER_SN=", cci_customer_SN_from_cmdline);
...
extern char disp_lcm_name[256];//justin temp add for read lcm name
static int prcb_lcm_proc_read(char *page, char **start, off_t off,
...
/*BUG1035 - HW/PRODECT ID Detection */
    create_proc_read_entry("CCI_HW_ID", 0, NULL, cci_hw_id_read, NULL);
    create_proc_read_entry("CCI_RF_ID", 0, NULL, cci_rf_id_read, NULL);
/*BUG1035 - HW/PRODECT ID Detection */

    create_proc_read_entry("CCI_COUNTRY_CODE", 0, NULL, cci_country_code_read, NULL);
...
#endif
        if (cci_customer_SN[0] != NULL)
			memcpy(serial_number, cci_customer_SN, SERIALNO_LEN);
		else
...
//#endif
	...
	create_proc_read_entry("lcm_vendor", 0644, NULL, prcb_lcm_proc_read, NULL);//justin temp add for read lcm name
