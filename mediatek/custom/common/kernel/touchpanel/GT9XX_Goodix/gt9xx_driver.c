...
// porting for touch driver start
//for dynamically opening log start
unsigned char debug_log_level = 0;
//for dynamically opening log end
//for reading raw data start
...
//for reading raw data end
// porting for touch driver end
volatile u8 g_sensor_id;
...
static ssize_t touch_firmware_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{

    s32 ret = -1;
    s32 i;
    u8 fw_buf[4] = {GTP_REG_FW_VERSION >> 8, GTP_REG_FW_VERSION & 0xff};
    u16 firmware_verison;	
//add sensor_ID start
    u8 sensor_id = 0;
//add sensor_ID end
    GTP_DEBUG_FUNC();

    ret = gtp_i2c_read(i2c_client_point, fw_buf, sizeof(fw_buf));
    ...
    firmware_verison = (fw_buf[3] << 8) | fw_buf[2];

//add sensor_ID start
    ret = gtp_i2c_read_dbl_check(i2c_client_point, GTP_REG_SENSOR_ID, &sensor_id, 1);
    if (SUCCESS == ret){
		if (sensor_id >= 0x06){
			GTP_ERROR("Invalid sensor_id(0x%02X)!", sensor_id);

			return sprintf(buf,"Invalid sensor_id(0x%02X)!\n", sensor_id);
			}
		}
    else{
		GTP_ERROR("Failed to get sensor_id!");

		return sprintf(buf,"Failed to get sensor_id!\n");
		}
    GTP_INFO("[#23]touch_firmware_version_show() Sensor_ID: 0x%x", sensor_id);
//add sensor_ID end

	GTP_ERROR("[#23]touch_firmware_version_show() tpd_info.vid = 0x%x, touch_firmware_version = 0x%x", tpd_info.vid, firmware_verison);

	return sprintf(buf,"touch_firmware_version = 0x%x, Sensor_ID = 0x%x\n", firmware_verison, sensor_id);
}

static ssize_t touch_firmware_version_check_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    u8 fw_buf[4] = {GTP_REG_FW_VERSION >> 8, GTP_REG_FW_VERSION & 0xff};
    u16 firmware_verison;	
    u16 shipping_firmware_verison = GTP_SHIPPING_FIRMWARE_VERSION;
    GTP_DEBUG_FUNC();

//add for GT928 FW check start
GTP_ERROR("[#23]touch_firmware_version_check_show() tpd_info.pid = 0x%x", tpd_info.pid);
	if(tpd_info.pid == 0x9280){
		shipping_firmware_verison = GTP_SHIPPING_FIRMWARE_VERSION_GT928;
		}
//add for GT928 FW check end
    ...
    GTP_ERROR("[#23]touch_firmware_version_check_show() shipping_firmware_verison = 0x%x, firmware_verison = 0x%x", shipping_firmware_verison, firmware_verison);
//

	if(firmware_verison == shipping_firmware_verison){
		//PASS
		//return sprintf(buf,"touch_firmware_version_check PASS!!!\n");
		return sprintf(buf, "Pass\n");
		}
	else{
		//FAIL
		//sprintf(buf, "%s", "Check TP shipping fw failed!\n");

		//TODO -- if FW version is too old, update FW
		return sprintf(buf,"touch_firmware_version_check FAIL, shipping_firmware_verison = 0x%x, firmware_verison = 0x%x\n", shipping_firmware_verison, firmware_verison);
		}
//
}
...
static ssize_t fw_upgrade_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	s32 ret = 0;
	u8 upguade_result = 1;
	gtp_FW_use_ARRAY =1; // use array for FW upgrade
	GTP_ERROR("fw_upgrade_show()  gtp_FW_use_ARRAY = %d\n", gtp_FW_use_ARRAY);
	
	ret = gup_init_update_proc(i2c_client_point);

	if (ret < 0){
		GTP_ERROR("Firmware upgrade fail\n");
		upguade_result = 0;
	}
	//else
		//GTP_ERROR("Firmware upgrade success\n");


	if(upguade_result)
	{
		return sprintf(buf, "1\n");//sprintf(buf, "FW upgrade SUCCESS\n");
	} else {
		return sprintf(buf, "0\n");//sprintf(buf, "FW upgrade FAIL\n");
	}

}

static ssize_t fw_upgrade_BIN_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	s32 ret = 0;
	u8 upguade_result = 1;
	gtp_FW_use_ARRAY =0; // use BIN for FW upgrade
	GTP_ERROR("fw_upgrade_BIN_show()  gtp_FW_use_ARRAY = %d\n", gtp_FW_use_ARRAY);
...
static ssize_t seneor_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    GTP_INFO("[#23]seneor_id_show() Sensor_ID: 0x%x", sensor_id);

    return sprintf(buf,"Sensor_ID = 0x%x\n", sensor_id);
}

static ssize_t product_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{

    s32 ret = -1;
    s32 i;
    u8 pid_buf[6] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};
    u16 PID = 0x00;
    GTP_DEBUG_FUNC();

    ret = gtp_i2c_read(i2c_client_point, pid_buf, sizeof(pid_buf));

    if (ret < 0)
    {
        GTP_ERROR("GTP read PID failed");
        return sprintf(buf,"GTP read PID failed\n");
    }

    for (i = 0; i < 4; i++)
    {
        if (pid_buf[i + 2] < 0x30)break;

        PID |= ((pid_buf[i + 2] - 0x30) << ((3 - i) * 4));
    }

	GTP_INFO("[#23]product_id_show() tpd_info.pid = 0x%x, PID = 0x%x", tpd_info.pid, PID);

	return sprintf(buf,"PID = 0x%x\n", PID);
}

static ssize_t tx_rx_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	int count = 0;
	...
	ret = gtp_parse_config(i2c_client_point);

	if (ret == FAIL)
	    count += sprintf(buf, "failed to parse config...\n");

	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);

	GTP_INFO("[#23]tx_rx_show() gt9xx_drv_num = %d, gt9xx_sen_num = %d", gt9xx_drv_num, gt9xx_sen_num);

	return sprintf(buf,"gt9xx_drv_num (Tx) = %d, gt9xx_sen_num (Rx)= %d\n", gt9xx_drv_num, gt9xx_sen_num);
}

static ssize_t reset_guitar_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int times = 0;
	int i =1;
	GTP_INFO("[#23]reset_guitar_store() enter\n");
	if(1 != sscanf(buf, "%d", &times))
	{
		GTP_ERROR("[#23]invalid format: '%s'\n", buf);
		return 0;
	}
	GTP_INFO("[#23]reset_guitar_store() times = %d", times);

	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);

	while(i <= times){
		GTP_INFO("[#23]reset_guitar_store() i = %d", i);
		i++;
...
//for dynamically opening log start
static ssize_t touch_debug_level_show(struct device *dev, struct device_attribute *attr, char *buf)
{
   return sprintf(buf, "touch_debug_level = %d\n", debug_log_level);
}
static ssize_t touch_debug_level_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    if (buf[0] >= '0' && buf[0] <= '255' && buf[1] == '\n')
        debug_log_level = buf[0] - 0x30;
    return count;
}
//for dynamically opening log end
...
static ssize_t raw_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (NULL == raw_buf)
    {
        count += sprintf(buf, "failed to allocate mem for raw_buf!\n");
        ret = FAIL;
        goto exit;
    }

    ret = gt9_read_raw_cmd(i2c_client_point); 
    if (ret == FAIL)
    {    
        count += sprintf(buf, "Send Read Rawdata Cmd failed!\n");
        ret = FAIL;
        goto exit;
    }
    ret = gtp_read_rawdata(i2c_client_point, raw_buf);
    if (ret == FAIL)
    {
        count += sprintf(buf, "Read Rawdata failed!\n");
        ret = FAIL;
        goto exit;
    }

    for(k=0,j = 0; k < (gt9xx_drv_num*gt9xx_sen_num); k++){
        count += sprintf(buf + count, "%4d ", raw_buf[k]);
        ++j;
...
static DEVICE_ATTR(touch_firmware_version, 0664, touch_firmware_version_show, NULL);
static DEVICE_ATTR(touch_firmware_version_check, 0664, touch_firmware_version_check_show, NULL);
static DEVICE_ATTR(fw_upgrade, 0664, fw_upgrade_show, NULL);
static DEVICE_ATTR(fw_upgrade_BIN, 0664, fw_upgrade_BIN_show, NULL);
static DEVICE_ATTR(seneor_id, 0664, seneor_id_show, NULL);
static DEVICE_ATTR(product_id, 0664, product_id_show, NULL);
static DEVICE_ATTR(tx_rx, 0664, tx_rx_show, NULL);
static DEVICE_ATTR(reset_guitar, 0664, NULL, reset_guitar_store);
//for dynamically opening log start
static DEVICE_ATTR(touch_debug_level, 0664, touch_debug_level_show, touch_debug_level_store);
//for dynamically opening log end
//for reading raw data start
static DEVICE_ATTR(raw_data, 0664, raw_data_show, NULL);
//for reading raw data end

static struct attribute *gt9xx_attrs[] = {
	&dev_attr_touch_firmware_version.attr,
	&dev_attr_touch_firmware_version_check.attr,
	&dev_attr_fw_upgrade.attr,
	&dev_attr_fw_upgrade_BIN.attr,
//for dynamically opening log start
    &dev_attr_touch_debug_level.attr,
    &dev_attr_raw_data.attr,
//for dynamically opening log end
	&dev_attr_seneor_id.attr,
	&dev_attr_product_id.attr,
	&dev_attr_tx_rx.attr,
	&dev_attr_reset_guitar.attr,
};
...
// porting for touch driver start
//Goodix add level trigger
    else if (GTP_INT_TRIGGER == 2)  //LOW
    {
        config[TRIGGER_LOC] &= 0xfc;
        config[TRIGGER_LOC] |= 0x02;
    }
    else if (GTP_INT_TRIGGER == 3)  //HIGH
    {
        config[TRIGGER_LOC] &= 0xfc;
...
#endif
GTP_ERROR("[#23]tpd_i2c_probe V2.0 enter");
    ...
    {
        GTP_ERROR("I2C communication ERROR!");
		mt_set_gpio_pull_select(GTP_INT_PORT, GPIO_PULL_UP); //set  GPIO_PULL_UP to avoid interrup trigger by mistake
		goto exit;
    }
    else
	GTP_ERROR("[#23]tpd_i2c_probe tpd_power_on() OK");
	...
	GTP_ERROR("[#23]tpd_i2c_probe gtp_read_version() OK");
	...
	GTP_ERROR("[#23]tpd_i2c_probe gtp_init_panel() OK");
...
//#if GTP_AUTO_UPDATE
#if 0 // porting for touch driver -- skip update
	...
	GTP_ERROR("[#23]tpd_i2c_probe gup_init_update_proc() OK");
#endif
...
//add device attribute
//=============================================================
for(i=0 ; i<(sizeof(gt9xx_attrs)/sizeof(gt9xx_attrs[0])) ; i++){
		ret = sysfs_create_file(&client->dev.kobj, gt9xx_attrs[i]);
		GTP_ERROR("[#23]tpd_i2c_probe sysfs_create_file ret = %d", ret);
}
//=============================================================
// porting for touch driver end

exit:
	GTP_ERROR("[#23]tpd_i2c_probe leave");
    return 0;
}
...
//    #if GTP_HAVE_TOUCH_KEY
        ...
        if(debug_log_level & 0x2)
            GTP_DEBUG("finger:0x%02x, key_value:0x%02x, pre_key:0x%02x\n", finger, key_value, pre_key);
...
// porting for touch driver start
        //for dynamically opening log start
        if(debug_log_level & 0x2)
            GTP_DEBUG("pre_touch:%02x, finger:%02x. level 2", pre_touch, finger);
        //for dynamically opening log end
// porting for touch driver end
        if (button_key_state == 0) {
        if (touch_num)
        {
...
// porting for touch driver start
                //for dynamically opening log start
                if(debug_log_level & 0x1)
                {
                    //GTP_DEBUG(" %d)(%d, %d)[%d]", id, input_x, input_y, input_w);
                    GTP_DEBUG(" %d)(%d, %d)[%d], level 1", id, input_x, input_y, input_w);
                }
                //for dynamically opening log end
// porting for touch driver end
...
// porting for touch driver start
				...
				if(debug_log_level & 0x1)
                {
                	GTP_DEBUG("Touch Release! (x.y)=%d,%d", input_x, input_y);
				}
// porting for touch driver end
                tpd_up(0, 0, 0);
            }
