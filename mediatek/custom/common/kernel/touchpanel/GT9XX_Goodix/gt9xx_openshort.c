...
#define  MAX_LIMIT_VALUE_MAIN_SOURCE  2700
#define  MIN_LIMIT_VALUE_MAIN_SOURCE  500
#define  MAX_LIMIT_KEY_MAIN_SOURCE    2400
#define  MIN_LIMIT_KEY_MAIN_SOURCE    450

#define  MAX_LIMIT_VALUE_SECOND_SOURCE  2700
#define  MIN_LIMIT_VALUE_SECOND_SOURCE  500
#define  MAX_LIMIT_KEY_SECOND_SOURCE    2400
#define  MIN_LIMIT_KEY_SECOND_SOURCE    450

u16 max_limit_value = 0;     // screen max limit
u16 min_limit_value = 0;     // screen min limit
...
#define tp_opentest_raw_file  "/data/tp_opentest_raw_file.txt"
#define tp_shorttest_raw_file  "/data/tp_shorttest_raw_file.txt"
...
extern u8 cfg_len;
extern int tpd_halt;
extern volatile u8 g_sensor_id;
...
//    gtp_hopping_switch(client, 0);
	...
	if((max_limit_value == 0) || (min_limit_value == 0))
	{
		if(g_sensor_id == 1)
		{
			max_limit_value = MAX_LIMIT_VALUE_MAIN_SOURCE;
			min_limit_value = MIN_LIMIT_VALUE_MAIN_SOURCE;
			max_limit_key = MAX_LIMIT_KEY_MAIN_SOURCE;
			min_limit_key = MIN_LIMIT_KEY_MAIN_SOURCE;
		}
		else if(g_sensor_id == 2)
		{
			max_limit_value = MAX_LIMIT_VALUE_SECOND_SOURCE;
			min_limit_value = MIN_LIMIT_VALUE_SECOND_SOURCE;
			max_limit_key = MAX_LIMIT_KEY_SECOND_SOURCE;
			min_limit_key = MIN_LIMIT_KEY_SECOND_SOURCE;
		}
		GTP_DEBUG("Sensor_ID: %d", g_sensor_id);
	}
...
// add start
//ret = 1; //LVP5 DVT1 will collect data, so show Pass only
	if(ret){
		//PASS
		len = 0;
		len += sprintf(&buf[len], "Pass\n");
