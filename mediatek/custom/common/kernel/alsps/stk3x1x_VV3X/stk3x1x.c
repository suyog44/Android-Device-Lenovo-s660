...
//#if (defined(MT6589) || defined(MT6572)) //VV38 porting for STK3310
//#include <mach/mt_devs.h>
#include <mach/mt_typedefs.h>
#include <mach/mt_gpio.h>
#include <mach/mt_pm_ldo.h>
//#endif //VV38 porting for STK3310

//#if ((defined MT6573) || (defined MT6575) || (defined MT6577) || (defined MT6589)  || (defined MT6572))	//VV38 porting for STK3310
...
struct stk3x1x_threshold {
    ...
    unsigned  int als_transmittance_cali; // add for FTM ALS CALI 20131024
    unsigned  int als_adc_cali;// add for FTM ALS CALI 20131024
    unsigned  int ps_high_thd;
    unsigned  int ps_low_thd;
};
...
//CCI Colby add start 20131024
//add for ALS cali
	als_ori = *data; //store ori als adc
	if(((*data)*cci_transmittance_cali) > 72088500){  //(((*data)*cci_transmittance_cali)/1100) > 65535
		*data = 65535;
		//APS_DBG("*data > 65535, set *data = 65535");
		}
	else
		*data = ((*data)*cci_transmittance_cali)/1100;
	if(CCI_debug_for_stk3x1x)
		APS_DBG("als_ori = %05d, cci_transmittance_cali = %d, *data = %05d.", als_ori, cci_transmittance_cali, *data);
//CCI Colby add end 20131024
...
int stk3x1x_cci_als_cali(int sSampleNo)
{
	...
	//cci_transmittance_cali = (500*1100)/als_reading;
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable)
{
	...
	//STK add for checking if the register value is ok 20130318 start
	u8  reg_wait;
	int err_wait, err_init;
	//STK add for checking if the register value is ok 20130318 end
	//STK add for CCI start 20130430 dust issue for variation in 2 consequential calls 
	unsigned int hang_up_hthd, hang_up_lthd, fac_ct;
	int ret;
	fac_ct = 2*cci_ps_low_thd-cci_ps_high_thd;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(ret == 0)
	{
		...
		printk(KERN_INFO "%s: show psa psi: psa=%d,psi=%d\n", __func__, obj->psa, obj->psi);		
...
static ssize_t stk3x1x_show_als(struct device_driver *ddri, char *buf)
{
	...
	{
		//return scnprintf(buf, PAGE_SIZE, "0x%04X\n", stk3x1x_obj->als);     
		return scnprintf(buf, PAGE_SIZE, "%05d\n", stk3x1x_obj->als);     // show decimal for als code
	}
}
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_ps(struct device_driver *ddri, char *buf)
{
	...
	{
		//return scnprintf(buf, PAGE_SIZE, "0x%04X\n", stk3x1x_obj->ps);     
		return scnprintf(buf, PAGE_SIZE, "%05d\n", stk3x1x_obj->ps);     // show decimal for ps code
	}
}
/*----------------------------------------------------------------------------*/
...
static ssize_t stk3x1x_show_nvdata(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[#23]cci_transmittance_cali =  %d\n", cci_transmittance_cali);
	APS_ERR("[#23]cci_als_adc_cali =  %d\n", cci_als_adc_cali);
	//scnprintf(buf, PAGE_SIZE, "%d\n", kd);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d, cci_transmittance_cali = %d, cci_als_adc_cali = %d\n", cci_ps_high_thd, cci_ps_low_thd, cci_result_ct, cci_ps_cover, cci_als_value, cci_transmittance_cali, cci_als_adc_cali);
}
//CCI Colby add end 20130111
/*----------------------------------------------------------------------------*/
//CCI Colby add start 20131022
...
//
	...
	if(ret > 0){
		APS_ERR("[#23]als_CCI_cali_Light_show() PASS!!! cci_transmittance_cali = %d\n", cci_transmittance_cali);
		return scnprintf(buf, PAGE_SIZE, "PASS!!! cci_transmittance_cali = %d\n", cci_transmittance_cali);
		}
	else{
		return scnprintf(buf, PAGE_SIZE, "FAIL.... ALS out of range, cannot do ALS calibration\n");
		}
//
	...
	int res;
	int32_t als_reading;
	APS_ERR("[#23]als_CCI_cali_Light_show\n");
	if(!stk3x1x_obj)
	{
		APS_ERR("stk3x1x_obj is null!!\n");
		return 0;
	}
	als_reading = stk3x1x_read_als_avg(5);

	//do ALS cali
	if(als_reading == 0)
		return scnprintf(buf, PAGE_SIZE, "als_reading = 0, cannot do ALS calibration\n"); 
	cci_transmittance_cali = (500*1100)/als_reading;
	if(cci_transmittance_cali == 0)
		cci_transmittance_cali = 1;
	APS_ERR("[#23]als_CCI_cali_Light_show() cci_transmittance_cali = %d, als_reading = %d\n", cci_transmittance_cali, als_reading);

	return scnprintf(buf, PAGE_SIZE, "cci_transmittance_cali = %d\n", cci_transmittance_cali);  		
#endif
}
//CCI Colby add end 20131022
/*----------------------------------------------------------------------------*/
...
//cci_transmittance_cali
//	u16 als_ori;
//	//APS_DBG("als = %05d, cci_transmittance_cali = %d", als, cci_transmittance_cali);
//	als_ori = als; //store ori als adc
//	als = (als*cci_transmittance_cali)/1100;
//	if(CCI_debug_for_stk3x1x)
//		APS_DBG("als_ori = %05d, cci_transmittance_cali = %d, als = %05d.", als_ori, cci_transmittance_cali, als);
//CCI Colby add end 20131022
...
#endif
{
	...
	int cci_als_cali_result;
	int cci_als_cali_data[2];
...
// add for FTM interrupt check 20130416 end		
// add for FTM ALS CALI 20131024 start
		case ALSPS_IOCTL_ALS_CALI:
			APS_ERR("%s ALSPS_IOCTL_ALS_CALI enter ", __FUNCTION__);

			//do ALS cali
			cci_als_cali_result = stk3x1x_cci_als_cali(5);
			if(cci_als_cali_result > 0){
				cci_als_cali_data[0] = cci_transmittance_cali;
				cci_als_cali_data[1] = cci_als_adc_cali;
				//cci_als_adc_cali
				APS_ERR("%s ALSPS_IOCTL_ALS_CALI PASS!!!! cci_transmittance_cali = %d, cci_als_adc_cali = %d", __FUNCTION__, cci_transmittance_cali, cci_als_adc_cali);
				}
			else{
				cci_als_cali_data[0] = -1;
				cci_als_cali_data[1] = cci_als_adc_cali;
				APS_ERR("%s ALSPS_IOCTL_ALS_CALI FAIL....", __FUNCTION__);
				}

			//send cci_transmittance_cali to FTM
			APS_ERR("%s ALSPS_IOCTL_ALS_CALI dat[0] = %d, dat[1] = %d", __FUNCTION__, cci_als_cali_data[0], cci_als_cali_data[1]);
			if(copy_to_user(ptr, &cci_als_cali_data, sizeof(cci_als_cali_data)))
			{
				err = -EFAULT;
				goto err_out;
			}
...
// add for FTM ALS CALI 20131024 end
		...
		case ALSPS_IOCTL_SET_CALI:
			...
			APS_ERR("%s ALSPS_SET_AUTO_CALI cci_ps_high_thd = %d, cci_ps_low_thd = %d", __FUNCTION__, cci_ps_high_thd, cci_ps_low_thd);
			...
			APS_ERR("%s ALSPS_SET_AUTO_CALI cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d", __FUNCTION__, cci_result_ct, cci_ps_cover, cci_als_value);

// add for FTM ALS CALI 20131024 start
			if(calibration_threshold.als_transmittance_cali > 0)
				cci_transmittance_cali = calibration_threshold.als_transmittance_cali;

			cci_als_adc_cali = calibration_threshold.als_adc_cali;
			APS_ERR("%s ALSPS_SET_AUTO_CALI als_transmittance_cali = %d, als_adc_cali = %d", __FUNCTION__, cci_transmittance_cali, cci_als_adc_cali);
// add for FTM ALS CALI 20131024 end
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			if(CCI_debug_for_stk3x1x)
				APS_ERR("[Colby]SENSOR_GET_DATA enter\n");		
			...
			else
			{
				...
				if((err = stk3x1x_read_ps(obj->client, &obj->ps)))
				{
					APS_ERR("[Colby]SENSOR_GET_DATA stk3x1x_read_ps error\n");					
					err = -1;
				}
				else
				{
					if(CCI_debug_for_stk3x1x)
						APS_ERR("[Colby]SENSOR_GET_DATA stk3x1x_read_ps ps = %d\n", obj->ps);					
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{				
				...
				if((err = stk3x1x_read_als(obj->client, &obj->als)))
				{
					APS_ERR("[Colby]SENSOR_GET_DATA stk3x1x_read_als error\n");					
					err = -1;
				}
				else
				{
					if(CCI_debug_for_stk3x1x)
						APS_ERR("[Colby]SENSOR_GET_DATA stk3x1x_read_als als = %d\n", obj->als);					
					sensor_data->values[0] = stk3x1x_get_als_value(obj, obj->als);
					sensor_data->value_divide = 1;
					sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
					if(CCI_debug_for_stk3x1x)
						APS_ERR("[Colby]SENSOR_GET_DATA values[0] = %d\n", sensor_data->values[0]);					
				}				
			}
			break;
		default:
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = stk3x1x_init_client(client)))
	{
	mt_set_gpio_pull_select(GPIO_ALS_EINT_PIN, GPIO_PULL_UP); //set GPIO_ALS_EINT_PIN as GPIO_PULL_UP to avoid interrup trigger by mistake
