...
int CCI_Mask_Temp=999;
int CCI_Charger_Switch=1;
int CCI_Temp_count=0;

#if defined(MTK_JEITA_STANDARD_SUPPORT)
  extern int CCI_recovery_time_high;
  extern int CCI_recovery_time_cool;
  extern int CCI_recovery_time_cold;
#endif
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_STATUS:
	     if (CCI_recovery_time_high == 30 || CCI_recovery_time_cool == 30 || CCI_recovery_time_cold == 60 || CCI_Charger_Switch == 0){
...
static ssize_t show_CCICurrent(struct device *dev,struct device_attribute *attr, char *buf)
{
	kal_int32 CCIvolt1=0;	
	kal_int32 CCIvolt2=0;
	kal_int32 CCIvolt3=0;
	kal_int32 CCIvolt4=0;
	kal_int32 CCIvolt5=0;
	kal_int32 CCIRes;
	kal_int32 CCIFTMCurrent;
	kal_uint32 charging_on;
	kal_uint32 set_current;
	int i;
	if (upmu_is_chr_det() == KAL_TRUE)
	{
		charging_on=KAL_FALSE;
		battery_charging_control(CHARGING_CMD_ENABLE,&charging_on);
		msleep(50);
		for (i=1;i<=10;i++)
		{
			CCIvolt1+=battery_meter_get_battery_voltage();
		}
		CCIvolt1=CCIvolt1/10;

	    battery_xlog_printk(BAT_LOG_CRTI, "[Battery] CCITestvolt1 : %d\n", CCIvolt1);

		charging_on=KAL_TRUE;
		battery_charging_control(CHARGING_CMD_ENABLE,&charging_on);

		msleep(50);
		set_current=CHARGE_CURRENT_100_00_MA;
		battery_charging_control(CHARGING_CMD_SET_CURRENT,&set_current);
		msleep(100);
		for (i=1;i<=10;i++)
		{
			CCIvolt4+=battery_meter_get_battery_voltage();
		}
		CCIvolt4=CCIvolt4/10;
		battery_xlog_printk(BAT_LOG_CRTI, "[Battery] CCITestvolt4 : %d\n", CCIvolt4);
		set_current=get_charging_setting_current();
		battery_charging_control(CHARGING_CMD_SET_CURRENT,&set_current);
		msleep(50);
		for (i=1;i<=10;i++)
		{
			CCIvolt2+=battery_meter_get_battery_voltage();
		}
		CCIvolt2=CCIvolt2/10;

	    battery_xlog_printk(BAT_LOG_CRTI, "[Battery] CCITestvolt2 : %d\n", CCIvolt2);

		CCIvolt5=CCIvolt4-CCIvolt1;
		CCIRes=((CCIvolt5*1000)/100)+80;
		
	    battery_xlog_printk(BAT_LOG_CRTI, "[Battery] CCIRes : %d\n", CCIRes);

		CCIvolt3=CCIvolt2-CCIvolt1;
		CCIFTMCurrent=(CCIvolt3*1000)/CCIRes;
    
    	return sprintf(buf, "%d",CCIFTMCurrent);
	}else{
    	return sprintf(buf, "0");
	}
}
static ssize_t store_CCICurrent(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    battery_xlog_printk(BAT_LOG_CRTI, "[Battery] CCITest\n");
    return size;
}
static DEVICE_ATTR(CCICurrent, 0664, show_CCICurrent, store_CCICurrent);

static ssize_t sculld_show_CCIMaskTemp(struct device *dev,struct device_attribute *attr,char *buf)

{
        return sprintf(buf,"%d\n",CCI_Mask_Temp);
}

static ssize_t sculld_store_CCIMaskTemp(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
        CCI_Mask_Temp = simple_strtol(buf, NULL, 0);
        return count;
}

static DEVICE_ATTR(CCIMaskTemp, 0664 , sculld_show_CCIMaskTemp, sculld_store_CCIMaskTemp);


static ssize_t sculld_show_CCIChargerSwitch(struct device *dev,struct device_attribute *attr,char *buf)

{
        return sprintf(buf,"%d\n",CCI_Charger_Switch);
}

static ssize_t sculld_store_CCIChargerSwitch(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
        CCI_Charger_Switch = simple_strtol(buf, NULL, 0);
        return count;
}

static DEVICE_ATTR(CCIChargerSwitch, 0664 , sculld_show_CCIChargerSwitch, sculld_store_CCIChargerSwitch);
...
static void mt_battery_notify_UI_test(void)
{
    ...
    else if(g_BN_TestMode == 0x0006)  /*[---Modify by Rachel, low temperature, 20130529---]*/
    {
        g_BatteryNotifyCode = 0x0020;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0002_VBATTEMP\n");
    }
    else if(g_BN_TestMode == 0x0007)  /*[---Modify by Rachel, bad battery, 20130529---]*/
    {
        g_BatteryNotifyCode = 0x0040;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0002_VBATTEMP\n");
    }
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    /* For EM */
	{
	    ...
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCICurrent);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCIMaskTemp);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCIChargerSwitch);
	}
