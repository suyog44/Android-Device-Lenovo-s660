...
#define MD1_TOPSM_ADDR_BASE 0x20030820
static unsigned char *md1_mcu_ptr; 
static unsigned char *md1_m3d_ptr; 
static unsigned char *md1_topsm_ptr;
#endif
...
static void map_md_side_register(int md_id)
{
	switch(md_id)
	{
		case MD_SYS1:
		  ...
		  if(md1_topsm_ptr==NULL)
				md1_topsm_ptr = ioremap_nocache(MD1_TOPSM_ADDR_BASE, 4);
#endif
			break;
...
void ccci_dump_md_register(int md_id)
{
  ...
  CCCI_MSG_INF(md_id, "cci", "Dump MD TOPSM register\n");
	map_ptr=md1_topsm_ptr;
	ptr=(unsigned char *)MD1_TOPSM_ADDR_BASE;
	CCCI_MSG_INF(md_id, "cci", "[%08X]->[%08X]=%08x\n",(unsigned int)(ptr), (unsigned int)(map_ptr) ,*((unsigned int*) (map_ptr)));
