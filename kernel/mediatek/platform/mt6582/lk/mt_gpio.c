...
s32 mt_set_gpio_pull_select_chip(u32 pin, u32 select)
{
	...
	for(i = 0; i < sizeof(kpad_pupd_spec)/sizeof(kpad_pupd_spec[0]); i++){
		if (pin == kpad_pupd_spec[i].pin){
			...
			if(pin == GPIO168)
			{
			        GPIO_SET_BITS((1L << 9), &obj->reg->kpad_ctrl[kpad_pupd_spec[i].reg].rst);
			        GPIO_SET_BITS((1L << 8), &obj->reg->kpad_ctrl[kpad_pupd_spec[i].reg].rst);
			}
			else if(pin == GPIO92)
			{
			        GPIO_SET_BITS((1L << 5), &obj->reg->kpad_ctrl[kpad_pupd_spec[i].reg].rst);
			        GPIO_SET_BITS((1L << 4), &obj->reg->kpad_ctrl[kpad_pupd_spec[i].reg].rst);
			}
